...
export type EditorTransaction = (state: EditorState) => TransactionResult;
...
export function buildDoc(lines: EditorLine[]): EditorDoc {
...
export function clampPosition(doc: EditorDoc, pos: LogicalPosition): LogicalPosition {
...
export function createEditorState(md: string, selectionOffset = 0): EditorState {
  const doc = markdownToDoc(md);
  return {
    doc,
    selection: createTextSelection(doc, selectionOffset),
    revision: 0,
    composing: false,
    sourceMode: false,
  };
